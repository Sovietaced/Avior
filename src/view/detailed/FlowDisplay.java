...
public class FlowDisplay {
		...
		protected void createContents() {
			...
			shell.setText("Flow Information for table " + tableID + " on switch " + selectedSwitch.getStringId());
			...
			btnDeleteFlows.addSelectionListener(new SelectionAdapter() {
				...
				public void widgetSelected(SelectionEvent e) {	
					...
					if(!Avior.getStaticFlowEntryPusher().getFlows(selectedSwitch.getStringId()).isEmpty()){
						Avior.getStaticFlowEntryPusher().deleteFlowsForSwitch(selectedSwitch.getId());
						Avior.getStaticFlowEntryPusher().getFlows(selectedSwitch.getStringId()).clear();
			...
			btnSave.addListener(SWT.Selection, new Listener() {
			      public void handleEvent(Event e) {
			    	...
			    	// Dispose the editor do it doesn't leave a ghost table item
				      ...
				      // Reset values to null now that the flow we're operating on is saved
			    	  selectedFlow = null;
			    	  currFlow = null;
			    	  ...
			    	  displayFlows();
			    		  ...
			    		  // This method parses the table data, saves it to the flow in real time, and returns
			    		  // the string value of the user gave the flow
			    		  String newFlowName = Flow.saveNew(currFlow, table_flow.getItems());
			    		...
			    		// Dispose the editor do it doesn't leave a ghost table item
			    		  ...
			    		  Avior.getStaticFlowEntryPusher().addFlow(newFlowName, currFlow, switchID);
			    		  newFlow = false;
			    		  currFlow = null;
			    		  displayFlows();	 
					       }
			...
			btnDelete.addListener(SWT.Selection, new Listener() {
			      public void handleEvent(Event e) {
			    	  ...
			    	  if(Avior.getStaticFlowEntryPusher().getFlows().get(selectedFlow) != null){
			...
			if(Avior.getStaticFlowEntryPusher().getFlows(selectedSwitch.getStringId()) != null){
				displayFlows();
			}
		}
}