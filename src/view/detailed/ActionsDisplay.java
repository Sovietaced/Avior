...
public class ActionsDisplay {
	...
	protected void displayActions(OFFlowMod selectedFlow) {
		...
		// Get the summaries of all the actions and populates the table with the
		// summaries
	...
	protected void createContents() {
		...
		combo.setItems(new String[] { "Output", "Enqueue", "Strip Vlan",
				"Set Vlan ID", "Set Vlan Priority", "Set Source Mac",
				"Set Destination Mac", "Set Tos Bits", "Set Source IP",
				"Set Desination IP", "Set Transport Source Port",
		...
		btnRemoveAllActions.addListener(SWT.Selection, new Listener() {
			public void handleEvent(Event e) {
				...
				if (flow.getActions() != null) {
					...
					Avior.getStaticFlowEntryPusher().addFlow(flowName, flow,
							switchID);
					displayActions(flow);
				}
			}
		...
		btnSave.addListener(SWT.Selection, new Listener() {
			public void handleEvent(Event e) {
				...
				if (currAction != null) {
					// Parse the changes to the table and update the flow's
					// actions accordingly
					FlowDisplay.currFlow.setActions(Action.saveExisting(
							table_action.getItems(), currAction, flow,
							currActionIndex));
					...
					// Clear the tables and refresh the information with the new
					// actions!
					displayActions(flow);
				} else if (newActionType != null) {
					// Parse the changes to the table and update the flow's
					// actions accordingly
					FlowDisplay.currFlow.setActions(Action.saveNew(
							table_action.getItems(), newActionType, flow));
